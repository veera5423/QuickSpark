  ...
  return (
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="text-center mb-8 max-w-3xl mx-auto">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-teal-600">Blog</p>
        <h1 className="mt-2 text-4xl font-black tracking-tight text-slate-950">QuickSpark Blog</h1>
        <p className="text-slate-600 mt-2">Insights, tutorials, and product updates to help you learn smarter.</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {posts.map((post) => (
          <div key={post.slug} className="block" role="button" tabIndex={0}
            ...
            <Card className="p-6 rounded-[2rem] border border-slate-200 hover:shadow-lg transition bg-white">
              <h2 className="text-2xl font-semibold mb-2 text-slate-950">{post.title}</h2>
              <div className="text-sm text-slate-500 mb-3">{post.date} • {post.author}</div>
              <p className="text-slate-700 mb-4">{post.excerpt}</p>
              <Link to={`/blog/${post.slug}`} className="text-teal-700 font-medium hover:underline">Read more →</Link>
            </Card>
          </div>
        ))}
      </div>
